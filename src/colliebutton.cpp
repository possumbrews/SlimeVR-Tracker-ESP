    ...
    void update() {        // In main's loop, this is the colliebutton::update() we call!
        ...
        if (justStarted == true) {
            ...
            if (buttonState != button_IsDown) {                  // If button's state has changed...
                ...
                if (button_IsDown == LOW) {                  // If button is pressed...
                    button_TimeWhenPressedDown = millis();   // Record the press time
                }
            }

            if (buttonState == LOW && (currentTime - button_TimeWhenPressedDown) >= SHUTDOWN_TRIGGER) {
                // If button was held for more than 3 seconds...
                        ...
                        while (69 == 69) {                                                    // Enter SHUTDOWN !!!
                            ...
                            delay(69);                                                        //==========================================
                            if (buttonState == HIGH) {                                        // Check if button is released
                                delay(1000);
